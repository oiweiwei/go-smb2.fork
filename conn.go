...
type Negotiator struct {
	...
	UseMessageEncryption  bool     // use encryption when supported?
...
retry:
	...
	conn.useEncryption = n.UseMessageEncryption && r.Capabilities()&SMB2_GLOBAL_CAP_ENCRYPTION != 0
...
func (conn *conn) makeRequestResponse(req Packet, tc *treeConn, ctx context.Context) (rr *requestResponse, err error) {
	...
	if s != nil {
		if _, ok := req.(*SessionSetupRequest); !ok {
			if s.conn.useEncryption || s.sessionFlags&SMB2_SESSION_FLAG_ENCRYPT_DATA != 0 || (tc != nil && tc.shareFlags&SMB2_SHAREFLAG_ENCRYPT_DATA != 0) {
